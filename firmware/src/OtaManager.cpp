...
void OtaManager::scanChunkForMarker(uint8_t* data, size_t len) {
  // Zusammengesetzter Suchpuffer: bisheriger Tail + neuer Chunk, aber nur
  // fuer die Prefix-Suche gebraucht - klein genug fuer einen Stack-Puffer.
  static const size_t kJoinCap = kTailCap + 512;
  if (!_capturing) {
    // Chunkweise durchsuchen, mit Tail-Ueberlappung fuer einen an der
    // Chunk-Grenze zerschnittenen Prefix.
