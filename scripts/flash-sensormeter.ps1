<#
.SYNOPSIS
  Installiert alle Abhaengigkeiten und flasht die Sensormeter-Firmware
  (WT32-ETH01) auf einem beliebigen Windows-PC.

.DESCRIPTION
  - Installiert Python + PlatformIO, falls nicht vorhanden (ueber winget/pip)
  - Installiert Git, falls nicht vorhanden (ueber winget)
  - Klont https://github.com/peterhagelhof7-cmd/sensormeter, falls unter
    -RepoPath noch kein Checkout liegt (bei vorhandenem, sauberem Checkout
    wird stattdessen "git pull" versucht)
  - Legt firmware/include/config.h aus der Vorlage an, falls sie fehlt
  ...
  Das Board muss dafuer am USB-Seriell-Adapter (Debug-Burning-Schnittstelle)
  angeschlossen sein, siehe docs/flash-vorbereitung.pdf im Repo.
...
.PARAMETER RepoPath
  ...
  Default: .\sensormeter neben diesem Skript.
...
.EXAMPLE
  .\flash-sensormeter.ps1

.EXAMPLE
  .\flash-sensormeter.ps1 -Port COM5

.EXAMPLE
  .\flash-sensormeter.ps1 -RepoPath C:\Projekte\sensormeter -SkipUpload
#>

[CmdletBinding()]
param(
  [string]$RepoPath = (Join-Path $PSScriptRoot "sensormeter"),
  [string]$Port,
  [switch]$SkipUpload
)
...
if (-not (Test-Path $configReal)) {
  ...
  Write-Host "include/config.h aus der Vorlage angelegt (Default-Firmwareversion, keine Zugangsdaten noetig)."
} else {
...
Write-Host ""
Write-Host "Hinweis: Das Board muss am USB-Seriell-Adapter (Debug-Burning-" -ForegroundColor Yellow
Write-Host "Schnittstelle, NICHT am 20-Pin-Hauptheader!) angeschlossen sein" -ForegroundColor Yellow
Write-Host "und sich im Boot-/Download-Modus befinden - siehe docs/flash-vorbereitung.pdf." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
  ...
  Write-Host "  - Board nicht im Bootloader-Modus (docs/flash-vorbereitung.pdf, Schritt 4)"
